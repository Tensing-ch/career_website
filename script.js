...
const jobs = [
  {
    ...
    resources: [
      ...
      { type: 'Playlist',  name: 'Software Engineering — YouTube Playlist', desc: 'Curated playlist covering core software engineering topics', free: true, icon: '▶️', color: 'rgba(234, 242, 240, 1)', url: 'https://www.youtube.com/watch?v=Ubg3PenEdNw&list=PLEiEAq2VkUUJfeP9bgWAEs-NbGJyYXc-v' },
    ],
  },
  {
    id: 'ds',
    ...
    resources: [
      { type: 'Course',   name: 'Data Science Bootcamp',   desc: 'Comprehensive data science training',             free: true,  icon: '🤖', color: 'rgba(0,212,255,0.15)', url: 'https://www.youtube.com/watch?v=xPh5ihBWang&t=424s' },
    ],
  },
  {
    id: 'devops',
    ...
    resources: [
      { type: 'course', name: 'Linux ',                   desc: 'Real time examples ',                 free: false, icon: '🐳', color: 'rgba(255,200,0,0.15)', url:  'https://www.youtube.com/watch?v=GzIFoJBVwh8&t=2042s'},
      { type: 'Course',   name: 'DevOps',                 desc: 'Hands-on DevOps learning path',            free: false, icon: '👨🏻‍💻', color: 'rgba(255,200,0,0.15)', url: 'https://www.youtube.com/watch?v=Ou9j73aWgyE&list=PLdpzxOOAlwvIKMhk8WhzN1pYoJ1YU8Csa' },
      { type: 'Course',   name: 'Ansible',                desc: 'Hands on labs ',    free: false, icon: '☁️', color: 'rgba(217, 255, 0, 0.15)', url: 'https://www.youtube.com/watch?v=O4i5Wh5JvyM&list=PLdpzxOOAlwvLxd5nmtmORCmhD5jkrNbuE' },
      { type: 'Course',   name: 'Kubernetes Docs',        desc: 'Hands on labs',               free: true,  icon: '⛵', color: 'rgba(204, 245, 0, 0.15)', url: 'https://www.youtube.com/watch?v=dfxrdoEQe00&list=PLdpzxOOAlwvJdsW6A0jCz_3VaANuFMLpc' },
    ],
  },
  {
    id: 'cybersec',
...
function updatePreview() {
  ...
  document.getElementById('pv-contact').innerHTML   = `<span>${email}</span> · <span>${phone}</span> · <span>${location}</span> · <span>${linkedin}</span>`;
      ...
      expHtml += `
        <div class="rv-exp-item">
          <div style="display:flex; justify-content:space-between; align-items:baseline;">
            <span class="rv-exp-title">${title}</span>
            <span class="rv-exp-duration">${duration}</span>
          </div>
          <div class="rv-exp-company">${company}</div>
  ...
  eduEntries.forEach(e => {
    ...
    if (degree || school) {
      eduHtml += `
        <div class="rv-exp-item">
          <div style="display:flex; justify-content:space-between; align-items:baseline;">
            <span class="rv-edu-degree">${degree}</span>
            <span class="rv-edu-year">${year}</span>
          </div>
          <div class="rv-edu-school">${school}</div>
        </div>`;
    }
  });
...
function downloadResume() {
  ...
  const css = [
    ...
    "body { font-family:'DM Sans',sans-serif; padding:40px; color:#1a1a1a; max-width:800px; margin:0 auto; line-height:1.5; }",
    ".rv-name { font-size:2.2rem; font-weight:700; color:#0f172a; letter-spacing:-0.03em; text-align:center; margin-bottom:2px; }",
    ".rv-role { font-size:0.95rem; font-weight:600; text-transform:uppercase; letter-spacing:0.08em; color:#0284c7; text-align:center; margin-bottom:12px; }",
    ".rv-contact { display:flex; justify-content:center; flex-wrap:wrap; gap:16px; font-size:0.78rem; color:#475569; margin-bottom:24px; padding-bottom:12px; border-bottom:1px solid #e2e8f0; }",
    ".rv-contact a { color:#475569; text-decoration:none; }",
    ".rv-section { margin-bottom:22px; }",
    ".rv-section-title { font-size:0.75rem; font-weight:700; letter-spacing:0.12em; text-transform:uppercase; color:#0f172a; margin-bottom:12px; padding-bottom:4px; border-bottom:1px solid #0f172a; }",
    ".rv-skill-wrap { display:flex; flex-wrap:wrap; gap:8px; }",
    ".rv-skill { font-size:0.72rem; padding:4px 10px; border-radius:4px; background:#f1f5f9; color:#334155; border:1px solid #e2e8f0; font-weight:500; }",
    ".rv-exp-item { margin-bottom:16px; }",
    ".rv-exp-title { font-weight:700; font-size:0.9rem; color:#0f172a; }",
    ".rv-exp-duration { font-size:0.8rem; color:#64748b; font-weight:500; }",
    ".rv-exp-company { font-size:0.8rem; font-style:italic; color:#475569; margin-top:1px; margin-bottom:6px; }",
    ".rv-exp-desc { font-size:0.8rem; color:#334155; line-height:1.5; white-space:pre-line; }",
    ".rv-edu-degree { font-weight:700; font-size:0.9rem; color:#0f172a; }",
    ".rv-edu-year { font-size:0.8rem; color:#64748b; font-weight:500; }",
    ".rv-edu-school { font-size:0.8rem; color:#475569; margin-top:1px; }",
    ".rv-summary { font-size:0.82rem; color:#334155; line-height:1.6; }"
  ].join('\n');
...
function toggleTheme() {
  const current = localStorage.getItem('site-theme') || (window.matchMedia('(prefers-color-scheme: light)').matches ? 'light' : 'dark');
